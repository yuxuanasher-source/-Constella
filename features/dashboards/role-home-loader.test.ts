...
describe("loadRoleHomeDashboard", () => {
  ...
  it("enriches zeroed project card placeholders with live dashboard facts", async () => {
    ...
    expect(dashboard.kpis).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ key: "vendorReceivable", value: 240 }),
        expect.objectContaining({ key: "estimatedGross", value: 160 }),
        expect.objectContaining({ key: "grossMarginRate", value: 66.7 }),
      ]),
    );
  });
