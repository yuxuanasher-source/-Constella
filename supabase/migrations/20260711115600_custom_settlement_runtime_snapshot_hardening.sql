-- Forward-only hardening for databases that already recorded 20260711115000.
-- The six-argument overload must lose every client ACL before it is removed.
do $snapshot_upgrade$
begin
  if pg_catalog.to_regprocedure(
    'public.read_custom_settlement_evidence_snapshot(uuid,uuid,text,date,date,integer)'
  ) is not null then
...
drop function if exists public.read_custom_settlement_evidence_snapshot(
  uuid,
  uuid,
  text,
  date,
...
-- Fail migration application if an overload survives or the hardened signature is absent.
do $snapshot_signature_check$
begin
  if pg_catalog.to_regprocedure(
    'public.read_custom_settlement_evidence_snapshot(uuid,uuid,text,date,date,integer)'
  ) is not null then
    raise exception 'custom_settlement_snapshot_legacy_signature_survived';
  end if;
  if pg_catalog.to_regprocedure(
    'public.read_custom_settlement_evidence_snapshot(uuid,uuid,text,date,date,text,text,text,integer,integer)'
  ) is null then
    raise exception 'custom_settlement_snapshot_hardened_signature_missing';
  end if;
end;
$snapshot_signature_check$;
