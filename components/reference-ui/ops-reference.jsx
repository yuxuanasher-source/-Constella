...
function buildReportMeaningText({
  ...
  const parts = [`当前状态是“${status.label}”，证据等级是 ${evidenceLevel}。`];
...
function ScreenWarRoom({ go }) {
  ...
  const reports = useOpsReports();
  const batches = useOpsSettlementBatches();
  const warRoomCurrentUser = useOpsCurrentUser();
...
function toSafeOcrJobView(job) {
  ...
  const metricCandidates = sanitizeOcrMetricCandidates(result.metricCandidates);
...
.streamer-pool-detail{min-width:0;position:sticky;top:76px;max-height:calc(100dvh - 96px);overflow-y:auto;overscroll-behavior:contain;scrollbar-gutter:stable;padding-right:4px;display:flex;flex-direction:column;gap:16px}
@container (max-width:1180px){.streamer-pool-layout{grid-template-columns:1fr}.streamer-pool-detail{position:static;max-height:none;overflow:visible;padding-right:0}.streamer-pool-filter-strip{flex-wrap:wrap}.streamer-pool-filter-spacer{display:none}}
        ...
        <>
          ...
          >
            ...
            s.matchTrend.some(hasStreamerMetricValue) ? (
              <Sparkline data={s.matchTrend} ariaLabel="近 6 周匹配分趋势图" />
            ) : (
        ...
        actions={
          <>
            ...
            <Button
              ...
              disabled={
                !autoReviewTargetReport || Boolean(autoReviewSubmitting)
              }
            >
          ...
          {autoReviewCandidateGroupVisible ? (
            ...
            >
              ...
              <Button
                ...
                onClick={() =>
                  setAutoReviewCandidatesExpanded((value) => !value)
                }
              >
      ...
      >
        ...
        <div style={{ padding: 16 }}>
          ...
          {screenshotUploadedAt || screenshotFileHash ? (
            ...
            >
              <span>上传 {formatEvidenceTimestamp(screenshotUploadedAt)}</span>
              {screenshotFileHash ? (
                <span
                  className="mono"
...
function AdmissionCalibrationDashboard({ metrics, loading, error, onRetry }) {
  const rows = latestAdmissionCalibrationRows(metrics);
  const overallAgreement = rows.find(
    (row) => row.metricKey === "ai_mcn_agreement" && !row.checkpointKey,
  );
  ...
  const worstFalsePass = rows
    .filter(
      (row) => row.metricKey === "ai_false_pass" && Boolean(row.checkpointKey),
    )
    .sort((left, right) => metricRate(right) - metricRate(left))[0];
  const vendorReasons = rows
    .filter(
      (row) =>
        row.metricKey === "vendor_reject_reason" && Boolean(row.checkpointKey),
    )
    .sort(
      (left, right) =>
  ...
  return (
    <Card
      title="组织审核校准"
      extra={latestPeriod ? <Badge tone="violet">{latestPeriod}</Badge> : null}
      padded={true}
    >
...
function OpsReferenceInner({
  ...
  const actions = React.useMemo(() => {
    ...
    const rotateAdmissionShareBoardToken = async (projectId, shareBoardId) => {
      return fetchJson(
        `/api/projects/${encodeURIComponent(
          projectId,
    ...
    const listAdmissionShareSubmissions = async (projectId, shareBoardId) => {
      const body = await fetchJson(
        `/api/projects/${encodeURIComponent(
          projectId,
        )}/admission-share-boards/${encodeURIComponent(
        ...
        >
          ...
          >
            ...
            {route === "knowledge" && (
              ...
              >
                <ScreenKnowledge dependencies={KNOWLEDGE_SCENE_DEPENDENCIES} />
              </Suspense>
            )}
            {route === "settle" && (
              <Suspense
...
const KNOWLEDGE_SCENE_DEPENDENCIES = {
  BlockEditor,
  Button,
  EmptyHint,
  Icon,
...
const SETTLEMENT_SCENE_DEPENDENCIES = {
  Avatar,
  BATCH_DETAIL_ITEMS,
  BATCH_STATUS,
  BATCHES,
