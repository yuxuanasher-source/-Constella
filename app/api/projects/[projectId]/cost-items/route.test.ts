import { beforeEach, describe, expect, it, vi } from "vitest";

import { GET, POST } from "./route";

import { assertBillingWriteAllowed } from "@/features/billing/route-guard";
import { getComplexCostRouteContext } from "@/features/complex-cost/complex-cost-route-utils";
import { createManualProjectCostItem } from "@/features/complex-cost/complex-cost-service";

vi.mock("@/features/billing/route-guard", () => ({
  assertBillingWriteAllowed: vi.fn(),
}));
vi.mock("@/features/complex-cost/complex-cost-service", () => ({
  createManualProjectCostItem: vi.fn(),
}));
vi.mock("@/features/complex-cost/complex-cost-route-utils", () => ({
  getComplexCostRouteContext: vi.fn(),
  complexCostActorFromContext: vi.fn((context) => context.auth),
  readJsonBody: async (request: Request) => request.json().catch(() => ({})),
  requiredString: (body: Record<string, unknown>, key: string) =>
    typeof body[key] === "string" ? (body[key] as string) : "",
  requiredNumber: (body: Record<string, unknown>, key: string) =>
    typeof body[key] === "number" ? (body[key] as number) : 0,
  optionalRecord: () => undefined,
  jsonError: (error: { message?: string; statusCode?: number }) =>
    Response.json(
      { error: error.message ?? "Unexpected error" },
      { status: error.statusCode ?? 500 },
    ),
}));

describe("project cost items route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(assertBillingWriteAllowed).mockResolvedValue(undefined);
    vi.mocked(getComplexCostRouteContext).mockResolvedValue({
      supabase: {},
      auth: {
        userId: "user-1",
        name: "Ops",
        role: "operator_business",
        organizationId: "org-1",
      },
      repo: { listProjectCostItems: vi.fn(async () => [{ id: "cost-1" }]) },
      audit: vi.fn(),
    } as never);
    vi.mocked(createManualProjectCostItem).mockResolvedValue({
      id: "cost-1",
      organizationId: "org-1",
      projectId: "project-1",
      itemType: "supplier_fee",
      amountCents: 12000,
      direction: "cost",
      evidenceLevel: "yellow",
      source: "manual",
      sourcePayload: {},
      reason: "Confirmed",
      status: "pending_review",
    });
  });

  it("lists project cost items", async () => {
    const response = await GET(new Request("http://localhost"), {
      params: Promise.resolve({ projectId: "project-1" }),
    });

    await expect(response.json()).resolves.toEqual({
      items: [{ id: "cost-1" }],
    });
  });

  it("creates a manual project cost item", async () => {
    const response = await POST(
      new Request("http://localhost", {
        method: "POST",
        body: JSON.stringify({
          itemType: "supplier_fee",
          amountCents: 12000,
          direction: "cost",
          evidenceLevel: "yellow",
          reason: "Confirmed",
        }),
      }),
      { params: Promise.resolve({ projectId: "project-1" }) },
    );

    expect(response.status).toBe(201);
    await expect(response.json()).resolves.toMatchObject({
      item: { id: "cost-1" },
    });
  });
});
