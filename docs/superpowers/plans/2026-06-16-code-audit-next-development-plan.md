...
本轮已完成的后续开发主要是风险收口，不是新增大功能。按当前代码状态估算，核心实现已经完成并通过全量验证，剩余成本集中在提交打包、是否纳入额外文档、以及上线后的观察。

| 项目                          | 当前状态                                            | 继续投入成本                 | 风险成本                                           |
| ----------------------------- | --------------------------------------------------- | ---------------------------- | -------------------------------------------------- |
| OAuth 回调与真实角色路由      | 已实现并通过 focused 测试                           | 低，主要是代码审阅和提交     | 中，若回调域名或环境变量配置错误，会影响第三方登录 |
| 移动端受保护路由回 `/m/login` | 已实现并通过 proxy 测试                             | 低                           | 低，主要影响移动端未登录跳转体验                   |
| 结算批次显式组织隔离          | 已实现并通过页面、API、query 测试                   | 低到中，建议重点审阅租户边界 | 中到高，结算数据越权属于高敏风险                   |
| PostCSS 安全修复与 CI audit   | 已实现，`pnpm audit:security` 通过                  | 低                           | 低，依赖 override 后需关注后续 Next 升级兼容       |
| 全量验证                      | 已通过 format、type-check、lint、test、build、audit | 低                           | 低，当前主要剩提交前范围确认                       |

建议的后续成本排布：

1. 提交前复核：约 10-20 分钟，确认 `docs/database-product-link-structure.md` 是否属于本批次。
2. 代码审阅：约 30-60 分钟，重点看 OAuth callback 的 `next` 白名单、结算 `organizationId` 传递链、CI audit 是否会被锁文件漂移影响。
3. 上线后观察：约 1 个发布窗口，关注第三方登录回调、移动端登录跳转、结算批次列表/详情读取。
4. 后续增强：约 0.5-1 天，补一个真实 Supabase/RLS 集成测试夹具，用来覆盖“同组织不同主播不能读 OCR job / 结算跨租户不可读”这类单测难以证明的边界。

结论：当前实现的剩余开发成本低，主要是交付包装成本；最高风险仍在租户隔离和认证回调配置，建议提交前做一次窄范围 code review。
