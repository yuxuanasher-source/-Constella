...
- 浏览器只提交候选主播 ID；`/api/war-room/matching` 按当前登录组织从服务端重载画像、完成率、选播通过率、ROI、毛利贡献、风险和可用时长，拒绝客户端自报候选指标。
- 服务端候选快照携带已确认的主播画像洞察，AI 简报与 Copilot 将其作为可追溯事实引用；画像缺失不会合成及格数据。
- 品类匹配加分。
- 平台匹配加分。
- 风格匹配加分。
