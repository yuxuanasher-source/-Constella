...
| 端             | 路由                      | 用户     | 产品任务                           |
| -------------- | ------------------------- | -------- | ---------------------------------- |
| 经营 Web       | `/console`                | MCN 员工 | 角色化今日待办与风险队列、模块导航 |
...
`/console` 默认进入角色化今日待办与风险队列，而不是在角色看板数据缺失时静默切到另一套首页。角色看板加载失败时，页面明确提示 `角色看板暂不可用`，并提供 `重新加载` 和 `进入作战台` 的手动入口；进入作战台是用户主动选择的降级路径。
...
目标：把已审核报数和人工确认的成本、分账动作转换成可追溯的财务批次。财务结算中心以财务批次为第一对象，批次按客户应收、主播应付、项目成本、协作分账分类型管理。项目不再限制批次范围，而是在每条明细和项目归因汇总中保留，用于收入、支出、成本、分账和毛利追溯。

主要功能：

- 读取已审核、未消费的结算来源。
- 创建财务批次。
- 生成客户应收、主播应付、项目成本、协作分账明细。
- 自动按已审核数据生成系统金额。
- 财务保存调整金额和调整原因。
- 按项目归因汇总批次收入、支出、成本和分账影响。
- 添加 CPA/CPS/礼物/手工金额承载项。
- 在复杂成本规则开通后，承载任务奖金、扣罚、补播、供应商费用、平台费、投流和游戏资产成本。
- 锁定批次。
- 重开已锁定批次。
- 主播端读取安全应付账单。
- 经营端读取财务批次列表、详情、项目归因和结算池预览。
...
结算、导出、OCR 人工确认等高风险动作在执行前需要先展示预检信息：动作范围、记录数量、字段范围、权限依据、审计去向和恢复路径。预检只解释后果和边界，不替代原因填写、权限校验或审计写入。
...
回答必须显示结论、来源、授权范围、数据更新时间、置信度、`需人工确认`、风险判断、人工处理建议和模块跳转。所有问答都会写入 AI 调用账本、AI 工具调用账本、用量和审计日志。AI 只给经营建议和跳转入口，不直接执行发布、审核、结算、导出或权限变更。
...
- AI 经营助理回答必须显示来源、授权范围、数据更新时间、置信度和 `需人工确认`。
- AI 只给经营建议和跳转入口，不直接执行发布、审核、结算、导出或权限变更。
...
| 表/视图                          | 用途                                           |
| --------------------------------- | ---------------------------------------------- |
| `finance_batches`                 | 统一承载客户应收、主播应付、项目成本、协作分账 |
| `finance_batch_items`             | 财务批次明细，保留项目、来源、对手方和证据快照 |
| `finance_batch_adjustments`       | 财务调整金额、原因和证据                       |
| `finance_batch_project_summary`   | 项目维度归因汇总，用于收入、支出、成本和分账追溯 |
